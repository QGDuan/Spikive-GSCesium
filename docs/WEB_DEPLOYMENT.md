# 面向建运一体化转型的实景三维多场景孪生应用底座系统——Web 安装与使用说明

系统采用前后端同源的 Web 部署方式。后端进程同时提供管理界面、API 和场景资源，不使用桌面安装器。

## 一、前置条件及环境配置

- 操作系统：macOS、Windows 10/11 x64，或常见 x64/arm64 Linux；
- Node.js `22.22.1` 或更高版本；
- 建议内存 16 GB 以上，并为原始 PLY、可视化产物和体素产物保留足够磁盘空间；
- 场景可视化使用访问者浏览器的 WebGPU/WebGL2；服务端体素计算还需要部署机可用的 WebGPU 适配器与显卡驱动。

默认端口为 `5173`，默认监听 `0.0.0.0`。局域网其他电脑访问时，需要在防火墙中允许该端口。

## 二、构建与启动

### 1. 在开发机生成 Web 安装包

```bash
npm ci
npm run package:web
```

命令会先生成 `release/web/` 部署目录，再生成：

- `release/packages/面向建运一体化转型的实景三维多场景孪生应用底座系统-<版本号>.zip`；
- `release/packages/SHA256SUMS.txt`；
- `release/packages/release-manifest.json`。

将 ZIP 复制到目标电脑并解压。压缩包不包含工程 TS/TSX、测试、Source Map、PLY、SQLite 或已有业务数据；客户后续上传的数据也保存在安装目录之外。

### 2. macOS / Linux 启动

```bash
cd web
./start.sh
```

### 3. Windows 启动

在资源管理器中双击 `start.cmd`，或在 PowerShell/CMD 中执行：

```bat
cd web
start.cmd
```

启动后，本机访问 `http://localhost:5173`；局域网访问使用 `http://服务器IP:5173`。服务在前台运行，按 `Ctrl+C` 停止。

### 4. 环境变量

| 变量 | 默认值 | 用途 |
|---|---|---|
| `SPIKIVE_HOST` | `0.0.0.0` | 监听地址；仅本机使用可设为 `127.0.0.1` |
| `SPIKIVE_PORT` | `5173` | HTTP 端口 |
| `SPIKIVE_DATA_ROOT` | 用户数据目录 | PLY、SOG、体素、SQLite 和任务数据根目录 |

默认数据位置：

- macOS/Linux：`~/.spikive-gs/data/`；
- Windows：`%LOCALAPPDATA%\Spikive GS\data\`。

### 5. 校验安装包

macOS/Linux 可在 `release/packages/` 中执行：

```bash
shasum -a 256 -c SHA256SUMS.txt
```

Windows PowerShell 可执行：

```powershell
Get-FileHash -Algorithm SHA256 '.\面向建运一体化转型的实景三维多场景孪生应用底座系统-0.1.1.zip'
```

输出值应与 `SHA256SUMS.txt` 一致。

## 三、业务使用与运维

### 1. 场景管理

1. 浏览器打开服务地址；
2. 在左侧卡片标题区选择“场景管理”；
3. 选择本机 PLY 文件，设置切片分级并上传；
4. 上传完成后点击“开始切片”，完成后点击“查看”；
5. 需要避障和航线规划时，再点击“计算体素”。

本次 `test` 优化版默认“低资源”构建：CPU 切片、各层串行，体素按分区逐一使用 GPU。构建期间本页面暂时卸载三维场景，完成后恢复镜头；请关闭其他大型三维网页。失败/取消后点“继续切片”或“继续体素任务”，相同参数可复用已完成且校验通过的阶段，不会自行降低清晰度或体素精度。

“显示体素”所需网格独立按需生成。分区版先选择调试分区编号，再点“生成该区调试网格”，生成后勾选显示。一次显示一个分区而非整场；网格失败不影响已完成的碰撞数据。卡片提供完整任务日志，排障时请发送完整日志，不只提供“退出码 1”截图。

16GB 内存／4GB 显存为优化验收目标，不是所有数据均可完成的保证。极端巨大或重叠 Gaussian、驱动挂起、磁盘不足仍会明确失败；软件不会静默改粗体素。需在客户实际 Windows 设备验证。

### 2. 标签管理

1. 必须先在“场景管理”中查看目标场景；
2. 在左侧卡片标题区选择“标签管理”；
3. 点击“在当前场景新建标签”，再到三维场景中点击目标表面；
4. 填写名称、说明和类型后保存；
5. 每个场景最多建立一个“起点”，被航线引用的标签不能直接删除。

### 3. 飞行路线

1. 场景必须已经生成体素，并建立一个起点和至少一个巡检标签；
2. 在左侧卡片标题区选择“飞行路线”；
3. 设置飞行速度、无人机膨胀系数、观察距离和点间距；
4. 按顺序选择巡检标签，建立航线并点击“计算航线”；
5. 规划成功后在三维场景中检查路线和航点。

### 4. 界面折叠与性能信息

- 左侧工作区右上角的箭头可以收起或展开场景、标签和航线卡片；
- 右下角性能卡片右上角的箭头可以收起或展开性能信息；
- 折叠只改变界面占用空间，不会停止三维场景、任务计算或性能采样；
- 性能卡片显示帧率、可见高斯点、系统处理器、系统内存和图形资源估算。

### 5. 备份与删除

- 备份前先停止服务，然后完整复制 `SPIKIVE_DATA_ROOT` 目录；
- 标签被航线引用时不能删除，必须先删除引用它的航线；
- 永久删除场景会同时删除其切片、体素、标签和航线，操作不可恢复。

### 安全边界

当前系统没有内置公网身份认证，默认只适合本机或可信局域网。不得将 `5173` 端口直接暴露到公网。如需公网或多用户使用，应在前置网关增加 HTTPS、身份认证、访问控制、上传限制和审计。
