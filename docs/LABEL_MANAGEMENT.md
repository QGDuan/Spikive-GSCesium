...
7. `label_references` 使用 `mission-start` / `mission-target` 记录航线引用。存在引用时标签拒绝删除；场景删除流程必须先删除其航线并释放引用。

航线、观察节点、引用和体素版本失效规则见 [`FLIGHT_ROUTE_PLANNING.md`](FLIGHT_ROUTE_PLANNING.md)。
