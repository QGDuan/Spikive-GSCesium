...
describe("route waypoint visuals", () => {
  it("renders label inspection points red with the durable hj name", () => {
    expect(routeWaypointVisual(waypoint("inspection"), 3, "配电箱")).toMatchObject({
      name: "hj_配电箱", color: "#e32636", showText: false
    });
  });

  it("renders generated transit points blue and Home neutral", () => {
    expect(routeWaypointVisual(waypoint("transit"), 4)).toMatchObject({ name: "4 transit", color: "#1677ff", showText: true });
    expect(routeWaypointVisual(waypoint("home"), 0)).toMatchObject({ name: "0 home", color: "#111318", showText: true });
  });

  it("renders a label-derived route origin red", () => {
    const start = { ...waypoint("home"), targetLabelId: "00000000-0000-4000-8000-000000000002" };
    expect(routeWaypointVisual(start, 0, "入口")).toMatchObject({ name: "hj_入口", color: "#e32636", showText: false });
  });
