...
interface Props {
  ...
  onPickLabel(position: Vec3): void;
  onMessage(message: string): void;
}

interface PickProbe { x: number; y: number }

export function CesiumScene({ dataset, labels, mission, labelMode, pendingPick, focusRequest, onPickLabel, onMessage }: Props) {
  ...
  useEffect(() => {
    ...
    setTilesRevision(current => dataset.status === "rebuilding" && current?.startsWith(prefix)
      ? current
      : `${prefix}${dataset.updatedAt}`);
  }, [dataset?.id, dataset?.status, dataset?.updatedAt]);
  ...
  useEffect(() => {
    ...
    const recoverRenderLoop = (_scene: unknown, error: unknown) => {
      ...
      if (tileset && !tileset.isDestroyed()) {
        // Reduce selected-tile churn after a transient Gaussian LOD resource race.
        tileset.maximumScreenSpaceError = Math.min(Math.max(tileset.maximumScreenSpaceError * 1.5, 32), 64);
        tileset.cullRequestsWhileMoving = false;
      }
      if (recentFailures.length > 3) {
        messageRef.current(`${tileset ? "GS" : "Cesium 场景"}连续渲染失败，已停止自动重试；完整错误已写入浏览器控制台`);
        return;
      }
      messageRef.current(tileset
        ? "GS LOD 切换出现瞬时资源冲突，正在降低细节抖动并恢复渲染…"
  ...
  useEffect(() => {
    ...
    void Cesium3DTileset.fromUrl(`/api/datasets/${dataset.id}/tiles/tileset.json?revision=${encodeURIComponent(tilesRevision)}`, {
      ...
      const restoreNormalTileset = () => {
        ...
        tileset.maximumScreenSpaceError = GS_STABLE_POLICY.maximumScreenSpaceError;
  ...
  useEffect(() => {
    ...
    if (pendingPick) {
      const position = localToWorld(pendingPick, transform);
      viewer.entities.add({
    ...
    for (const label of sceneLabels) {
      ...
      viewer.entities.add({ id: `app:label:${label.id}`, position, point: { pixelSize: 11, color: Color.fromCssColorString(label.color), outlineColor: Color.WHITE, outlineWidth: 2, disableDepthTestDistance: Number.POSITIVE_INFINITY }, label: { text: label.title, font: "14px sans-serif", style: LabelStyle.FILL_AND_OUTLINE, outlineWidth: 3, verticalOrigin: VerticalOrigin.BOTTOM, pixelOffset: new Cartesian2(0, -14), disableDepthTestDistance: Number.POSITIVE_INFINITY } });
    ...
    if (sceneMission?.waypoints.length) {
      ...
      sceneMission.waypoints.forEach((point, index) => {
        ...
        viewer.entities.add({ id: `app:waypoint:${point.id}`, name: visual.name, position: positions[index], point: { pixelSize: visual.pixelSize, color: waypointColor, outlineColor: Color.WHITE, outlineWidth: 2, disableDepthTestDistance: Number.POSITIVE_INFINITY }, label: { text: visual.name, font: visual.emphasized ? "bold 13px sans-serif" : "12px sans-serif", fillColor: waypointColor, style: LabelStyle.FILL_AND_OUTLINE, outlineColor: Color.WHITE, outlineWidth: 3, pixelOffset: new Cartesian2(0, -13), disableDepthTestDistance: Number.POSITIVE_INFINITY } });
      });
    }
  }, [dataset?.id, placementKey, labels, mission, pendingPick]);
...
const GS_STABLE_POLICY = Object.freeze({ maximumScreenSpaceError: 16 });
